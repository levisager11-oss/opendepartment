  ...
  return (
    <article className="animate-fade-up group relative">
      ...
      <div className="paper-tab ml-4 inline-block px-3 py-0.5">
        <span className="docket !text-[0.6rem] !text-ink-700">
          {caseLabel(file.case_number, branding.docketPrefix)}
        </span>
      </div>

      <div className="paper relative flex gap-3 rounded-xs p-3 transition-shadow group-hover:shadow-md">
        <VoteButtons
          fileId={file.id}
        ...
        >
          <div className="relative h-20 w-20 shrink-0 overflow-hidden rounded-xs border border-paper-400 bg-paper-200">
            {thumbnail ? (
          ...
          <div className="min-w-0 flex-1">
            <div className="mb-1 flex flex-wrap items-center gap-2">
              <span className="stamp stamp-blue !border-2 !px-1.5 !py-0 !text-[0.55rem]">
                {file.category}
              </span>
              {file.subjects.slice(0, 2).map((s) => (
                <span key={s.id} className="docket !text-[0.6rem] !text-gov-800">
                  · {s.name}
                </span>
              ))}
            ...
            <h3 className="truncate font-[family-name:var(--font-serif)] text-base font-bold text-gov-900 group-hover:underline">
              {file.title}
            </h3>
            ...
            <div className="docket mt-2 flex flex-wrap items-center gap-x-3 gap-y-1 !text-[0.6rem]">
              <span className="typewriter !text-[0.7rem] !normal-case !tracking-normal !text-gov-800">
                {file.owner_username ?? "—"}
              </span>
