...
export function AdminStorage() {
  ...
  const load = useCallback(async () => {
    ...
    // A department that has not re-run the schema since these were added has
    // no such function, which is a missing upgrade rather than an error.
    if (orphanResult.error) {
      setUnavailable(true);
      setOrphans([]);
    } else {
      setOrphans((orphanResult.data ?? []) as Orphan[]);
    }
  ...
  async function purge() {
    ...
    setNote(t("settings.orphansPurged", { n: orphans.length }));
    void load();
  }
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-6">
          ...
          <div>
            ...
            {unavailable ? (
              <p className="notice notice-error text-xs">
                {t("settings.orphansUnavailable")}
              </p>
